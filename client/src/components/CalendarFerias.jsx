...
const CalendarFerias = ({ handleChange, inserted, feriados, ferias,DiasCount,inserting, compensacao, compensacaoDomingo, inicio, fim, objetivo,aceitacao, vProjeto, todos, numberUsers, horasExtraID, selectedDate , user }) => {
    ...
    const calWeekDays1 = [ "Segunda", "Terça", "Quarta", "Quinta", "Sexta", "Sábado", "Domingo"];
    ...
    useEffect(() => {
        initializeCalendar();
      },  [selectedDate,inserted?.length, ferias?.length, inicio,inserting, vProjeto, user, type]);
    ...
    }, [selectedDate, inicio, vProjeto,inserted, inserting, user, ferias?.length, type]);
    ...
    function plotDayNames() {
        ...
        for (let i = 0; i < calWeekDays1.length ; i++) {
            const day = document.createElement("div");

            day.textContent = calWeekDays1[i];
            calendarBody.appendChild(day);
            }
        }
    }
    ...
    function highlightInsertedDays(calendar) {
        ...
        if (todos && todos === true) {
            ...
            const listaDias = ferias.map(({ Data }) => { 
                ...
                return { Data: Data, isHighlighted: count };
            });
            // Sort the list in ascending order
            const sortedFerias = listaDias.sort((a, b) => a.date - b.date);
 
            if (sortedFerias) {
           
                for (let i = 0; i < sortedFerias.length; i++) {
                    const insertedDay = new Date(sortedFerias[i].Data);
                    const currentMonth = insertedDay?.getMonth() + 1;
                    const currentYear = insertedDay?.getFullYear();
                    const isHighlighted = sortedFerias[i].isHighlighted;
                    if (
                        currentYear === changedYear &&
                        currentMonth === changedMonth &&
                        numberItems !== null &&
                        ...
                        // Set the formatted string to state
                         ...
                         numberItems[insertedDay.getDate() - 1].classList.add("calendar-ferias");
                            ...
                            numberItems[insertedDay.getDate() - 1].classList.add("calendar-rep2");                     
                        } else if(isHighlighted === 3) {
                            numberItems[insertedDay.getDate() - 1].classList.add("calendar-rep3");                     
                        } else if(isHighlighted === 4) {
                            numberItems[insertedDay.getDate() - 1].classList.add("calendar-rep4");                     
                        } else if(isHighlighted === 5) {
                            numberItems[insertedDay.getDate() - 1].classList.add("calendar-rep5");                     
                        } else if(isHighlighted > 5) {
                            numberItems[insertedDay.getDate() - 1].classList.add("calendar-rep6");      
                        }
                    }
                }
            }
    ...
    return (
        <Wrapper>
            ...
            <div className="row"  style={{ margin: "auto" }}>
                <div className="col-9 text-end" style={{ margin: "auto" }}>
                    <p>Alterar Calendário</p>
                     </div>

            <div className="col-3 text-start" style={{ margin: "auto" }}>
            <label className="switch" style={{ margin: "10%"}}>
                ...
                <div className="calendar-inner">
                    ...
                    <div className="calendar-controls">
                        <div className="calendar-prev text-end col-md-12" style={{ margin: "auto", height:"100%" }}>
                            <button className=".calendar .calendar-prev"    style={{ margin: "auto", height:"100%" }} onClick={() => navigateToPreviousMonth()}>
                        ...
                        <div className="calendar-next text-start col-md-12" style={{ margin: "auto", height:"100%" }}>
                            <button className=".calendar .calendar-next a " style={{ margin: "auto",height:"100%"}} onClick={navigateToNextMonth}>
                                <MdKeyboardArrowRight width="128" height="128" />
                            </button>
                        </div>
                    </div>
                    <div className="calendar-today-date"  onClick={navigateToCurrentMonth}>
                        {type === true ? calWeekDays[localDate.getDay()] : calWeekDays1[localDate.getDay()]} {localDate.getDate()} {calMonthName[localDate.getMonth()]} {localDate.getFullYear()}
                    </div>
